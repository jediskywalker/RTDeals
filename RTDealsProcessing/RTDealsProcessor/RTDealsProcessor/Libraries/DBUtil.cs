...
namespace Libraries
{
    public class DBUtil
    {
        private static string ConnectionString()
        {
            ...
            {
                LogUtil.Log(LogLevel.CRITICAL, "DB.ConnectionString", "Missing DBConnStr", null);
                throw (new Exception("Missing connection string DBConnStr"));
            }
            return connstr;
        }
        ...
        private static MySqlConnection OpenConnection()
        {
            ...
            {
                LogUtil.Log(LogLevel.CRITICAL, "DB.OpenConnection", "Failed to open connection for " + connstr, ex);
        ...
        //Execute MySqlCommand to return a SqlDataReader
        ...
        public static int Insert<T>(T model, string tablename, string identityColumn)
        {
            ...
            List<TableColumnInfo> columns = (List<TableColumnInfo>)CacheUtil.GetCache(cacheKey);
            if (columns == null)
            {
                columns = GetTableColumnInfo(tablename);
                CacheUtil.SetCache("TableColumnInfo-" + tablename, columns, -1);
            }

            // get type T properties
            cacheKey = "PropertyInfo-" + typeof(T).ToString();
            PropertyInfo[] properties = (PropertyInfo[])CacheUtil.GetCache(cacheKey);
            if (properties == null)
            {
                properties = typeof(T).GetProperties();
                CacheUtil.SetCache(cacheKey, properties, -1);
            }
